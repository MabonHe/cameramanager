...
if [ ! -z $MEDIA_CONFIG_COMMAND ] ; then
    ...
    if [ "${CAMERA_NUM}" == "1" ] || [ -z ${CAMERA_NUM} ];then
        ...
        # for 720x576, fps target is 50fps.
        if [ "$I_RESOLUTION" == "720x576" ];then
            GTEST_CASE_FILTER=${GTEST_CASE_FILTER//360_FPS_60/360_FPS_50}
        fi
        echo 'Run test app'
        echo "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
        ...
        do
            ...
            # for 720x576, fps taget is 50
            if [ "${I_RESOLUTION[$i]}" == "720x576" ];then
                GTEST_CASE[$i]=${GTEST_CASE_FILTER//360_FPS_60/360_FPS_50}
            else
                GTEST_CASE[$i]=${GTEST_CASE_FILTER}
            fi
            echo "App command: ${SW_VAL_ROOT}/bin/ipu4_v4l2_test --gtest_filter=${GTEST_CASE[$i]} -d=$dev_name -i=$is_block_mode -f=$field_order \
            -m=$memory_type -p=$pixel_format -w=$width -h=$height -c="pwd" -r=${DUMP_PREFIX[$i]} -export_device=${export_device[$i]}"
            ${SW_VAL_ROOT}/bin/ipu4_v4l2_test --gtest_filter=${GTEST_CASE[$i]} -d=$dev_name -i=$is_block_mode -f=$field_order \
