...
function get_mondello_command_according_res_info()
{
    ...
    if [ $resolution == '1920x1080' ]; then
        if [ $iformat == 'yuv422' ] || [ $iformat == 'uyvy' ] ; then
        ...
        elif [ $iformat == 'yuv422_yuyv' ] || [ $iformat == 'yuyv' ] || [ $iformat == 'nv16' ]; then
            # 1920x1080 yuv422_yuyv interlaced
            if [ $is_interlaced == 'true' ]; then
                MONDELLO_COMMAND="yuv422_yuyv_1080i_2lane.py"
            # 1920x1080 yuv422_yuyv progressive
            else
                MONDELLO_COMMAND="yuv422_yuyv_1080p_4lane.py"
            fi    
        ...
        elif [ $iformat == 'cvbs' ];then
            MONDELLO_COMMAND="cvbs_tx_a_1_lane_Ain_1.py"    
        fi
        ...
        if [ $iformat == 'yuv422' ] || [ $iformat == 'uyvy' ]; then
            ...
            # 720x576 yuv422_yuyv interlaced
            if [ $is_interlaced == 'true' ]; then
                MONDELLO_COMMAND="yuv422_yuyv_576i_1lane.py"
            # 720x576 yuv422_yuyv progressive
            else
                MONDELLO_COMMAND="yuv422_yuyv_576p_1lane.py"
            fi    
        elif [ $iformat == 'rgb24' ] || [ "$iformat" == "rgb888" ]; then
        ...
        if [ $iformat == 'yuv422' ] || [ $iformat == 'uyvy' ]; then
            ...
            # 640x480 yuv422_yuyv progressive
            if [ $is_interlaced == 'false' ]; then
                MONDELLO_COMMAND="yuv422_yuyv_vga_1lane.py"
            fi
        elif [ $iformat == 'rgb24' ] || [ "$iformat" == "rgb888" ]; then
        ...
        if [ $iformat == 'yuv422' ] || [ $iformat == 'uyvy' ]; then
            ...
            # 1280x720 yuv422_yuyv progressive
            if [ $is_interlaced == 'false' ]; then
                MONDELLO_COMMAND="yuv422_yuyv_720p_4lane.py"
            fi
        elif [ "$iformat" == "rgb24" ] || [ "$iformat" == "rgb888" ]; then
        ...
        if [ $iformat == 'yuv422' ] || [ $iformat == 'uyvy' ] ; then
            ...
            # 720x480 yuv422 interlaced
            if [ $is_interlaced == 'true' ]; then
                MONDELLO_COMMAND="yuv422_yuyv_480i_1lane.py"
            fi
        elif [ $iformat == 'rgb24' ] || [ "$iformat" == "rgb888" ]; then
...
function send_command_to_mondello_server()
{
    ...
    if [ ! -z "$MONDELLO_COMMAND" ] ; then
        echo "stream off mondello"
        echo mondello_streamoff.py > /dev/udp/$MONDELLO_SERVER_IP/13579
        sleep 2
