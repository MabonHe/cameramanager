...
if [ "$CAMERA_NUM" == "1" ];then
    ...
    do
        ...
        if [ `echo $CAM_CONFIG | grep -i "interlaced" | wc -l` -gt 0 ];then
            IS_INTERLACED[$i]="true"
            FILED_ORDER[$i]=V4L2_FIELD_ALTERNATE
        fi
        if [ `echo $CAM_CONFIG | grep -i "dmabuf" | wc -l` -gt 0 ];then
            I_MEMORY_TYPE[$i]="dmabuf"
        elif [ `echo $CAM_CONFIG | grep -i "mmap" | wc -l` -gt 0 ];then
            I_MEMORY_TYPE[$i]="mmap"
        fi    
        echo "I_RESOLUTION[$i]=${I_RESOLUTION[$i]}, I_PIXEL_FORMAT[$i]=${I_PIXEL_FORMAT[$i]}, I_MEMORY_TYPE[$i]=${I_MEMORY_TYPE[$i]}, I_IS_BLOCK_MODE[$i]=${I_IS_BLOCK_MODE[$i]}, IS_INTERLACED[$i]=${IS_INTERLACED[$i]}, FILED_ORDER[$i]=${FILED_ORDER[$i]}, WIDTH[$i]=${WIDTH[$i]}, HEIGHT[$i]=${HEIGHT[$i]}"
        i=$((i+1))
    done
else
    ...
    if [ ! -z ${HEIGHT[$i]} ] ; then
        ...
        if [ ! -z ${I_PIXEL_FORMAT[$i]} ] ; then
            if [ "${I_PIXEL_FORMAT[$i]}" == "raw8" ] ; then
                ...
                PIXEL_FORMAT[$i]=V4L2_PIX_FMT_NV16
                FORMAT[$i]=YUYV
