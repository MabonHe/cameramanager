...
if [ "$CAMERA_NUM" == "1" ];then
    ...
    if [ $I_TEST_TYPE == "60fps" ]; then
        ...
        if [ "$PROJECT_CODE" == "BMW"  ] && [ "$I_PIXEL_FORMAT" == "uyvy" ] && [ "$interlace_mode" == "false" ]; then
            gstsink="tee name=t ! queue ! mfxvpp ! mfxsink t. ! queue ! filesink location=${DUMP_FILE_NAME}"
        elif [ "$I_PIXEL_FORMAT" == "nv16" ];then
            gstsink="tee name=t ! queue ! videoconvert ! ximagesink t. ! queue ! filesink location=${DUMP_FILE_NAME}"
        else
            gstsink="tee name=t ! queue ! videoconvert ! autovideosink t. ! queue ! filesink location=${DUMP_FILE_NAME}"
        fi
    ...
    elif [ $I_TEST_TYPE == "printfield" ]; then
        printfps=false
        num_buffers=100
        printfield=true
        DUMP_FILE_NAME="${CASE_NAME}.raw"
    ...
    if [ "$I_PIXEL_FORMAT" == "uyvy" ] || [ "$I_PIXEL_FORMAT" == "yuy2" ] || [ "$I_PIXEL_FORMAT" == "yuyv" ] || [ "$I_INTERLACED" == "interlaced" -a  "$I_PIXEL_FORMAT" != "nv16" ];then
    ...
    if [ ! -z $I_DEINTERLACE ] && [ $I_DEINTERLACE == 'deinterlace' ]; then
        ...
        if [ `echo ${CASE_NAME_LOWER} | grep "sw_weaving" | wc -l` -gt 0 ];then
            deinterlace_method=sw_weaving
        fi
        if [ `echo ${CASE_NAME_LOWER} | grep "hw_weaving" | wc -l` -gt 0 ];then
            deinterlace_method=hw_weaving
        fi
    fi
elif [ "$CAMERA_NUM" == "2" ] || [ "$CAMERA_NUM" == "4" ];then
    I_TEST_TYPE=`echo $CASE_NAME_LOWER | awk -F_ '{print $(3)}'`
    if [ "$CAMERA_NUM" == "2" ];then
        . ${SW_VAL_ROOT}/scripts/resolution_list/resolution_list.sh adv7481_dual.sh
    ...
    do
        ...
        if [ "${I_RESOLUTION[$i]}" = "xres" ];then
            I_RESOLUTION[$i]=$(get_random_res)
            echo "Get random resolution: ${I_RESOLUTION[$i]}"
        fi
        ...
        io_mode[$i]=0
        # since the interlaced camera is real analog sensor, no analyzers. We direct to display.
        if [ `echo $CAM_CONFIG | grep -i "interlaced" | wc -l` -gt 0 ];then
            I_INTERLACED[$i]="interlaced"
            if [ "${I_PIXEL_FORMAT[$i]}" = "cvbs" ];then
                DUMP_FILE_NAME[$i]=
                gstsink[$i]="vaapipostproc ! vaapisink"
            fi
            deinterlace_method[$i]=sw_bob
            if [ `echo ${CAM_CONFIG} | grep "sw_weaving" | wc -l` -gt 0 ];then
                deinterlace_method[$i]=sw_weaving
            fi
            if [ `echo ${CAM_CONFIG} | grep "hw_weaving" | wc -l` -gt 0 ];then
                deinterlace_method[$i]=hw_weaving
            fi
        fi
        if [ `echo $CAM_CONFIG | grep -i "dmabuf" | wc -l` -gt 0 ];then
            io_mode[$i]=3
            gstsink[$i]=" vaapipostproc ! ${gstsink[$i]}"
        elif [ `echo $CAM_CONFIG | grep -i "mmap" | wc -l` -gt 0 ];then
            io_mode[$i]=1
        fi
        echo "I_RESOLUTION[$i]=${I_RESOLUTION[$i]}, I_PIXEL_FORMAT[$i]=${I_PIXEL_FORMAT[$i]},WIDTH[$i]=${WIDTH[$i]}, HEIGHT[$i]=${HEIGHT[$i]}, I_INTERLACED[$i]=${I_INTERLACED[$i]}, DUMP_FILE_NAME[$i]=${DUMP_FILE_NAME[$i]}, gstsink[$i]=${gstsink[$i]}, io_mode[$i]=${io_mode[$i]}"
        i=$((i+1))
    done
...
do
    if [ "${I_PIXEL_FORMAT[$i]}" == "uyvy" ]; then
        FORMAT[$i]=UYVY
        if [ "$cameraInput" == "mondello" ]; then
            MONDELLO_FORMAT[$i]=uyvy
        fi
    elif [ "${I_PIXEL_FORMAT[$i]}" == "yuyv" ] || [ "${I_PIXEL_FORMAT[$i]}" == "yuy2" ]; then
        FORMAT[$i]=YUY2
        if [ "$cameraInput" == "mondello" ]; then
            MONDELLO_FORMAT[$i]=yuyv
        fi
    elif [ "${I_PIXEL_FORMAT[$i]}" == "rgb24" ]  || [ "${I_PIXEL_FORMAT[$i]}" == "rgb888" ]; then
        #progressive use old format， interlaced use new format!!
        #progressive is from the Front End, its 24bit BGR
        #interlaced and dmabuf share mode is from the BackEnd, its 32bit BGRx
        if [ "${I_INTERLACED[$i]}" == "interlaced" ] || [ "${io_mode[$i]}" == "3" ]; then
            FORMAT[$i]=BGRx
        else
            FORMAT[$i]=BGR
        fi
        ...
        FORMAT[$i]=NV12
        MONDELLO_FORMAT[$i]=''
    # NV16 is newly supported format. The mondello output 
    elif [ ${I_PIXEL_FORMAT[$i]} == 'nv16' ];then
        FORMAT[$i]=NV16
        if [ "$cameraInput" == "mondello" ]; then
            MONDELLO_FORMAT[$i]=uyvy
        fi
    # cvbs is analog camera.
    elif [ ${I_PIXEL_FORMAT[$i]} == 'cvbs' ];then
        FORMAT[$i]=UYVY
        if [ "$cameraInput" == "mondello" ]; then
